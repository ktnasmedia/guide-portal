...
def parse_blocks(text, heads):
    ...
    #  (1) '공개일' 레이블 뒤의 값(날짜/미정/N월)까지 포함 — 사이에 '|' 구분자가 껴도 건너뜀
    ...
    while i < n:
        ...
        cut = False
        if label == "공개일":
            # '공개일' 뒤에 구분자(|/빈칸)가 끼어 있어도 건너뛰고, 가까운 곳(최대 2줄)의
            # 공개일 값(날짜/미정/N월)까지 블록에 포함한다. 값이 아니면(다음 작품 제목 등)
            # 끌어오지 않고 그대로 끊는다.
            j = i + 1
            steps = 0
            while j < n and steps < 3:
                if is_separator(lines[j]):
                    ...
                    steps += 1
                    continue
                if is_release_value(lines[j]):
                    cur.append(lines[j])
                    i = j
                break
            cut = True
        elif DATE_RE.search(ln) and len(ln) <= 16:
            # 정확한 날짜로 끝나는 작품 (신작·주목 섹션)
            cut = True
        if cut:
